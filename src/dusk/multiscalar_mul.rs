...
fn scalar_to_le_u64s(scalar: &Scalar) -> [u64; 4] {
...
fn to_radix_2w(scalar: &Scalar, w: usize) -> [i8; 43] {
    ...
    let scalar64x4 = scalar_to_le_u64s(scalar);
...
mod tests {
    ...
    #[test]
    fn pippenger_boundary_outputs_match_legacy() {
        use core::fmt::Write;
        ...
        let scalars = (0..800)
            .map(|i| x + (Scalar::from(i as u64) * y))
            .collect::<Vec<_>>();
        let premultiplied = scalars
            .iter()
            .zip(&points)
            .map(|(scalar, point)| point * scalar)
            .collect::<Vec<_>>();

        // Compressed outputs generated by the byteorder implementation.
        let cases = [
            (
                499,
                "b3d8de40321920efdd1b93dac1b0f77238c783b41e92a83233685ad33f4d1285a5d00615ffde457ab1c3b1a6dcf03a79",
            ),
            (
                500,
                "880ca077374e5220b1a6c36c0191f3f20c3073980310a04ffe3f24725f2c2624f6bda6a7b876fb2ead3acafb63ec7bde",
            ),
            (
                799,
                "99e53378487f1dbcf8e658f7c00861eb6d46b09a07266b36bc365128912b1c6a9462ef5b226fb0cbe3c6e2f22a4e1982",
            ),
            (
                800,
                "86b2fbc7f29541ce27f6e39e1bf5f42c6b354ed4b247ce05ce5cf9d8f99b9b655fe122cdedeca5a6d43b49659942b97c",
            ),
        ];

        for (n, expected) in cases {
            let control: G1Projective = premultiplied[..n].iter().sum();
            let subject = pippenger(points[..n].iter().copied(), scalars[..n].iter().copied());
            assert_eq!(subject, control);

            let mut compressed = String::new();
            for byte in G1Affine::from(subject).to_compressed() {
                write!(compressed, "{byte:02x}").unwrap();
            }
    ...
    #[test]
    fn scalar_little_endian_limbs_cover_edges() {
        let maximum_bytes = [
        ...
        let maximum = Scalar::from_bytes(&maximum_bytes).unwrap();

        assert_eq!(scalar_to_le_u64s(&Scalar::zero()), [0; 4]);
        assert_eq!(scalar_to_le_u64s(&Scalar::one()), [1, 0, 0, 0]);
        assert_eq!(
            scalar_to_le_u64s(&maximum),
            [
